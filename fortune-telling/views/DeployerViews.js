...
exports.WaitingForFortune = class extends React.Component {
  render() {
    return (
      <div>
        Waiting for fortune revealing...
      </div>
    );
  }
}

exports.DecideFortuneTruthy = class extends React.Component {
  render() {
    const {parent, playable, fortune} = this.props;
    return (
      <div>
        {fortune ? 'You denied! Pick again.' : ''}
        <br />
        {!playable ? 'Please wait...' : ''}
        <br />
        <button
          disabled={!playable}
          onClick={() => parent.makeDecision(false)}
        >False</button>
        <button
          disabled={!playable}
          onClick={() => parent.makeDecision(true)}
        >True</button>
      </div>
    );
  }
